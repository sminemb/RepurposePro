...
is the visual source of truth for RepurposePro UI implementation.

Coding agents must implement the UI **exactly as shown in the provided design references** unless an explicit user instruction overrides them.

Before implementing any screen, component, or interaction, the agent must inspect the relevant files in:

```text
docs/design/
```

The agent must match, as closely as technically practical:
...
- Form composition
- Icon placement
- Responsive behavior shown or implied by the design
- Overall visual density and composition

Do not invent a different layout simply because another pattern is easier to code.

Do not redesign the UI unless explicitly instructed.

Do not substitute generic shadcn/ui defaults when the provided design shows a different composition.

Use shadcn/ui primitives as implementation building blocks, but style and compose them to match the design reference.

When implementing from a design reference:

```text
1. Inspect the relevant file in docs/design/
2. Identify the screen and its components
3. Compare against ui-tokens.md
4. Compare against ui-rules.md
5. Compare against ui-registry.md
6. Implement the visual structure
7. Verify spacing, alignment, hierarchy, and responsive behavior
8. Record any unavoidable deviation in progress-tracker.md
```

If a design and another UI document conflict:

```text
docs/design/ wins for visual appearance
ui-rules.md wins for interaction and behavior
ui-tokens.md wins for reusable token definitions unless the design clearly requires a documented update
api-contracts.md and database-schema.md remain authoritative for data and backend behavior
```

Any intentional deviation from `docs/design/` must be:

- Necessary for accessibility, correctness, or technical feasibility
- Minimal
- Documented in `progress-tracker.md`
- Explained in the task handoff

Agents must not claim a UI task is complete until the implemented screen has been visually checked against the corresponding design reference.

Recommended completion check:

```text
Design reference opened
-> implementation rendered
-> side-by-side comparison performed
-> major visual mismatches fixed
...
## 13.1 Design Folder Implementation Rule

When implementing frontend UI, agents must implement the interface **exactly as shown in the project design references**.

Canonical design reference location:

```text
docs/design/
```

Agents must inspect the relevant files in `docs/design/` before building or modifying any UI screen or component.
...
Do not substitute a different layout because it is easier to implement.

Do not create a new visual direction unless explicitly instructed.

If there is a conflict between `docs/design/` and the written UI docs, prefer:
...
If the design is missing a state, follow `ui-rules.md` and `ui-registry.md` while preserving the visual style from `docs/design/`.

Before marking any UI task complete, the agent must verify:

- [ ] The implemented screen matches the relevant design file.
- [ ] Layout spacing matches the design.
- [ ] Colors match the design tokens.
- [ ] Typography is consistent with the design.
- [ ] Buttons and controls are in the same locations.
- [ ] Empty/loading/error states follow the same visual system.
- [ ] Responsive behavior does not break the design intent.
- [ ] Any intentional deviation is documented in `progress-tracker.md`.

Record design verification in the task execution log:
...
If no matching design exists in `docs/design/`, the agent must:

1. Use `ui-rules.md`, `ui-registry.md`, and `ui-tokens.md`.
2. Match the closest existing design pattern.
3. Record the missing design reference in `progress-tracker.md`.
